...
#!/usr/bin/env python
# coding: utf-8
from Resnet_cnn2 import *
from Resnet_cnn2 import ResNet18, ResNet50
import numpy as np
import torch
...
for epoch in range(num_epochs):
    ...
    for minibatch_count, (images, labels) in enumerate(train_loader, 0):
        ...
        if minibatch_count % N == N - 1:    
            ...
            N_minibatch_loss /= N
            print('Epoch %d, newest per-minibatch training loss: %.3f' %
                (epoch + 1, N_minibatch_loss))
            ...
            with torch.no_grad():
                ...
                print("minibatch count: ", minibatch_count + 1)
                i = 0
                best_val_loss = 1e6 
                ratio_val_used = 10   # 1/ratio_val_used of val set is used
                flag = True
                ...
                print("Per-minibatch validation loss:", best_val_loss)
...
def tester(model, test_loader, batch_size):
    with torch.no_grad():
        ...
        aggre_accu, aggre_recall, aggre_preci = 0.0, 0.0, 0.0
        print("Accuracy:", end = ' ') 
        for i in range(14): 
...
def ThresholdMoving(train_loader):
...
def visualize_filter(f, channel = 0, nrow = 8, padding = 2):
    f = f.cpu()
    n, c, h, w = f.shape
    if c != 3:
        f = f[:, channel,:,:].unsqueeze(dim = 1)
    rows = np.min((f.shape[0] // nrow + 1, 64 ))  
    grid = utils.make_grid(f, nrow = nrow, normalize = True, padding = padding)
...
m2 = model.double()  
body = [i for i in m2.children()][0]
layer = body
filterre = (layer.weight.data.clone())[:16]
visualize_filter(filterre)
